...
import main.java.RTedesco.Ametralladora;
import main.java.RTedesco.Buque;
import main.java.RTedesco.Cuchillo;
import main.java.RTedesco.Escudo;
import main.java.RTedesco.Museo;
import main.java.RTedesco.RecursoMuseo;
import main.java.RTedesco.Soldado;
import main.java.RTedesco.Tanque;
...
public class App {
    ...
    public void debeCrearSoldado() {
     ...
     private void assertEquals(int i, Object ataque) {
    }

    private void assertEquals(double e, Object vida, Object object) {
    }

    private void assertEquals(String string, Object nombre) {
    }
    ...
    public void debeCrearSuperTest() {
        ...
        Museo m = new Museo();
        Escudo escudo = new Escudo(0.5f);
        Soldado IronMan = new Soldado("Rafiterminaitor ", 1000, 1000);
    ...
    public void imprimirsolosoldadocon00(){
        ...
        for (int i = 0; i < 10000; i++) {
            Soldado soldado = new Soldado("Rafiterminaitor"+i, 10, 1);
            Soldados.add(soldado);
        }
        final Predicate<Soldado> termina00 = s -> ((String) s.getNombre()).endsWith("00");
    ...
    public void nuemosPares(){
        ...
        Predicate<Soldado> numerospares =  s -> !(((String) s.getNombre()).contains( "1") || ((String) s.getNombre()).contains( "3") || ((String) s.getNombre()).contains( "5") || ((String) s.getNombre()).contains( "7") || ((String) s.getNombre()).contains( "9"));
    ...
    Predicate<Soldado> LongitudImpar = s -> ((String) s.getNombre()).length() % 2 != 0;
    ...
    public void uniTestUnitario() {
        ...
        Predicate<Soldado> DigitosIguales = s -> ((String) s.getNombre()).charAt(((String) s.getNombre()).length() - 1) == ((String) s.getNombre())
                .charAt(((String) s.getNombre()).length() - 2)
                && ((String) s.getNombre()).charAt(((String) s.getNombre()).length() - 1) == ((String) s.getNombre()).charAt(((String) s.getNombre()).length() - 3);

        Predicate<Soldado> Termina00 = s -> ((String) s.getNombre()).endsWith("00");

        Predicate<Soldado> DigitosPares = s -> ((String) s.getNombre()).charAt(((String) s.getNombre()).length() - 1) % 2 == 0
                && ((String) s.getNombre()).charAt(((String) s.getNombre()).length() - 2) % 2 == 0
                && ((String) s.getNombre()).charAt(((String) s.getNombre()).length() - 3) % 2 == 0
                && ((String) s.getNombre()).charAt(((String) s.getNombre()).length() - 4) % 2 == 0;
